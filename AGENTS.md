...
4. Fato de caso concreto é hipótese de teorema, nunca `axiom`.
5. Escolha interpretativa é `Prop` com nome; nunca embutida no corpo de uma `def`.
6. Redação normativa entra como `Version` com texto literal, `textHash` (`sha256:` do UTF-8) e testemunha. Sem Leizilla, marcar `Origin.fixture`.
7. Mudança material de sentido gera declaração nova; a antiga vira `deprecated` (ADR-0004).
8. Não afirmar que compilar prova correção jurídica. Não afirmar pioneirismo (ver `docs/prior-art.md`).
9. Não construir aqui o que é de Leizilla, CausaGanha, Papers ou Skills (ADR-0001, `docs/ecosystem.md`).
