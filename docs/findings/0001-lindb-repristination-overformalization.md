...
- **Regra interpretativa, separada:** `Interpretive.contrary_provision_restores`. Havendo disposição em contrário, `a` se restaura. O texto abre a exceção; dizer que ela basta é interpretação.
- **As duas juntas:** `restored_by_lapse_iff_contrary` dá a equivalência. O `#premises` mostra que ela depende de ambas e de lógica clássica.
