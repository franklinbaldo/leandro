...
**LegalLean** — [github.com/edu-ap/legal-lean](https://github.com/edu-ap/legal-lean) (MIT, Lean 4, criado em 2026). Projeto distinto do Legalean acima. Codifica leis como tipos verificáveis em arquitetura de três camadas (Lean, grafo de argumentos em JSON, monografia), com `EvidenceFor`, regras derrotáveis (`DefeasiblyHolds`) e, principalmente, `FormalisationBoundary` e `RequiresHumanDetermination`, que representam no próprio sistema de tipos a fronteira entre o formalmente verificável e a determinação humana (textura aberta de Hart). Estudos de caso: IRC §163(h) completo; outros em esboço. *É prior art direto da tese central do LeanDRO:* a separação entre derivação formal e juízo jurídico já existe codificada em Lean 4. A diferença pretendida está em proveniência documental (URN LEX, redação, hash, testemunha), temporalidade e no livro-razão de premissas auditado no build. As tabelas comparativas publicadas pelo próprio LegalLean vêm de pesquisa com LLM que o README admite poder refletir viés a favor do projeto, e não são usadas aqui como evidência independente. Correção: a versão anterior deste documento dizia "não localizado", por falha da busca.
...
O programa de pesquisa mantido separadamente (pipeline Argdown → Lean → revisão substantiva, ainda sem publicação) e a skill `legal-argument-lean` já usam `#print axioms` como livro-razão de premissas. O `#premises` do LeanDRO é uma versão verificável dessa prática, não uma ideia nova.
